...
$totalPoints = $pts ? (int) $pts['total_points'] : 0;

$paymentDetail = '';
if (($order['payment_type'] ?? '') === 'fpx' && !empty($order['fpx_bank'])) {
  $paymentDetail = 'FPX — ' . $order['fpx_bank'];
} elseif (!empty($order['card_number_masked'])) {
  $brand = $order['card_brand'] ?? 'Card';
  $paymentDetail = $brand . ' ' . $order['card_number_masked'];
  if (!empty($order['card_holder'])) {
    $paymentDetail .= ' (' . $order['card_holder'] . ')';
  }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <style>
    ...
    .points-banner .earned {
      font-size: 13px;
      color: #2a6e00;
    }

    .points-banner .total-pts {
      font-size: 18px;
      font-weight: 700;
      color: #2a6e00;
    }
...
<body>
  ...
  <div class="page-wrap" id="receiptPrintArea">
    ...
    <div class="receipt-card">
      <div class="receipt-header">
        <img src="img/blank-logo.png" alt="Blank Perfume" class="logo-print">
      ...
      <div class="info-grid">
        ...
        <div class="info-box">
          ...
          <?php if ($paymentDetail): ?>
            <div style="font-size:12px;color:#666;margin-top:4px;font-weight:500;"><?php echo htmlspecialchars($paymentDetail); ?></div>
          <?php endif; ?>
        </div>
        <div class="info-box">
        ...
        <?php if (!empty($order['delivery_name'])): ?>
          <div class="info-box full">
            ...
            <div class="val"><?php echo htmlspecialchars($order['delivery_name']); ?></div>
            <div style="font-size:13px;color:#666;font-weight:400;margin-top:4px;">
              <?php echo htmlspecialchars($order['delivery_phone'] ?? ''); ?> · <?php echo htmlspecialchars($order['delivery_email'] ?? ''); ?>
            </div>
            <div style="font-size:13px;color:#666;font-weight:400;margin-top:4px;"><?php echo nl2br(htmlspecialchars($order['delivery_address'] ?? '')); ?></div>
          </div>
        <?php endif; ?>
      </div>
      ...
      <?php while ($item = $items->fetch_assoc()): ?>
        <div class="item-row">
          <div class="item-img"><img src="<?php echo htmlspecialchars(product_image_src($item['image'])); ?>" alt=""></div>
          <div style="flex:1;">
            <div style="font-weight:600;"><?php echo htmlspecialchars($item['name']); ?></div>
            <div style="font-size:12px;color:#999;"><?php echo htmlspecialchars($item['variant']); ?> × <?php echo (int) $item['quantity']; ?></div>
          </div>
      ...
      <div class="totals-section">
        ...
        <?php if (!empty($order['points_discount']) && $order['points_discount'] > 0): ?>
          <div class="total-row discount"><span>Points (<?php echo (int) ($order['points_redeemed'] ?? 0); ?> pts)</span><span>− RM <?php echo number_format($order['points_discount'], 2); ?></span></div>
        <?php endif; ?>
        <?php if ($order['discount'] > 0): ?>
          <div class="total-row discount"><span>Voucher <?php echo $order['voucher_code'] ? '(' . htmlspecialchars($order['voucher_code']) . ')' : ''; ?></span><span>− RM <?php echo number_format($order['discount'], 2); ?></span></div>
        <?php endif; ?>
      ...
      <div class="points-banner">
        <div>
          <div class="earned">⭐ You earned <strong><?php echo (int) $order['points_earned']; ?></strong> points from this order</div>
      ...
      <div class="action-row no-print">
        ...
        <button type="button" class="btn-black" onclick="window.print()"><i class="bi bi-printer"></i> Print Receipt</button>
      </div>
